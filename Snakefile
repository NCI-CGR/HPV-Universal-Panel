...
def parse_sampleID(filename):
    ...
    return base.split('.')[0] # HPV31_SCD4560.tmap.bam
...
TARGETS = ['reports/type_summary_perc.tsv',
            ]
...
rule fasta:
    ...
    output:
        'fasta/{sampleID}_HPV%s.fasta' %config['hpv_type']
    run:
...
rule fix_pileup:
    ...
    output: 'adjusted_pile/{sampleID}.no_pad.pileup'
    run:
        df = pandas.read_table(input[0], names=['chrom', 'pos', 'nt', 'cov', 'qual1', 'qual2'], sep='\t', index_col=False)
        df['new_pos'] = df.apply(lambda x: x['pos']-bed[x['chrom']] if x['pos'] > bed[x['chrom']] else x['pos'], axis=1)
        df = df[['chrom', 'new_pos', 'cov']].copy()
        df.set_index(['chrom', 'new_pos']).sum(level=0).reset_index() # combine rows with the same pos
        df = df[['chrom', 'new_pos', 'cov']]
        df.columns = ['chrom', 'pos', 'cov'] # rename new_pos to match other rules (yeah, being lazy)
        df.to_csv(output[0], sep='\t', index=False)

#--------------------------------------------------------------------------
rule type_summary:
    input: expand('adjusted_pile/{sampleID}.no_pad.pileup', sampleID=d.keys())
    output: 'reports/type_summary.tsv'
    run:
        stacks = []
        # iterate through each sample pileup and pull out which types were found
        for sample in input:
            df = pandas.read_table(sample, sep='\t', index_col=False)
            print(df.head())
            df = df[df['cov'].astype(int) >= int(config['min_read'])]
...
#--------------------------------------------------------------------------
rule type_summary_percent:
    input: 'reports/type_summary.tsv'
    output: 'reports/type_summary_perc.tsv'
    params: bedcov = 'bedcov.txt'
    priority: 50
    run:
        shell('bedtools coverage -a {len_bed} -b {amp_bed} > {params.bedcov}')
        df = pandas.read_table(input[0], sep='\t', index_col='sampleID')
        dcov = pandas.read_table(params.bedcov, sep='\t', index_col='chr', names=['chr', 'start', 'end', 'features', 'cov', 'a', 'b'])
        cols = df.columns.tolist()
        for col in cols:
            ampcov = dcov.loc[col]['cov']
            df[col + '_percent'] = df[col].apply(lambda x: x/ampcov * 100)

        newcol = [x + '_percent' for x in cols]
        df = df[newcol]
